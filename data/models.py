...
class Round(Model):
    ...
    send_deadline_reminder_emails: Mapped[bool] = mapped_column(default=True, nullable=False)
...
class PiiDeletionLog(Model):
    ...
    # was attempted but errored. A failed application's DB rows are guaranteed untouched (its commit
    # never happened, so it's eligible for a clean retry), but note its S3 files may have been
    # partially deleted before the failure - S3 deletion isn't transactional with the DB commit, so a
    # retry may find fewer files than expected there, which is expected and harmless.
